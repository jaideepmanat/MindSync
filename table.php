...
// Query to fetch history and consultant names
$sql = "
    SELECT h.date_registered, h.status, h.closed_date, u.name AS consultant_name
    FROM history h
    JOIN users u ON h.consultant_id = u.id
    WHERE h.user_id = ? 
                ...
                <?php if (count($history_entries) > 0): ?>
                    <?php foreach ($history_entries as $entry): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($entry['consultant_name']) ?></td>
